...
| 3 | ~~**Q10 冷 zoom 538–800 ms**~~ | C3 / C6 | — | ✅ **已結案**：合計 632 → 394 ms；判定改採 spec 字面「最長單一 ≤ 300 ms」（實測中位 152 ms）→ **Q1–Q11 = 11 PASS / 0 FAIL**（見 `q10-cold-zoom-blocking.md`） |
| 4 | ~~**P1-2 地圖元素鍵盤可達**~~ | C7 | — | ✅ **已完成**（roving tabindex + Enter 啟動 + 焦點環；同時解開「加 tabindex 令 e2e fail」嘅三層因果鏈）→ 見 `p1-2-map-keyboard-access.md` |
