...
| 唔應該做 | 應該做 |
|---|---|
| ❌ 建議「隨機抽 30 條核對」 | ✅ 用**程式化驗證**（schema、語義約束、交叉檢查） |
| ❌ 要求用戶人手覆核 LLM 輸出 | ✅ 用**多代理交叉驗證**或**確定性規則** |
| ❌ 停低等用戶確認中間結果 | ✅ 自主完成，只喺**缺密碼／授權／硬件**時停 |
| ❌ 設計「review queue」等人手處理 | ✅ 自動化全部驗證步驟 |
| ❌ 報告嘅「下一步」寫「人手覆核」 | ✅ 寫「擴充自動驗證規則」「加多啲語義約束」 |

**理由**：呢個專案嘅目標係**完全自動化嘅資料管線**。人手步驟會成為
瓶頸，而且唔可重跑。所有驗證都要寫入 script（可重跑、可稽核）。
